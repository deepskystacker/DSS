...
inline void CDrawContext::DrawText( int x, int y, LPCTSTR pcszText, size_t nLength, COLORREF crFore )
	{
		...
		VAPI( ::ExtTextOut( m_hdc, x, y, 0, NULL, pcszText, static_cast< UINT >( nLength ),NULL) );
	}
...
inline void CDrawContext::SetClipRegion( HRGN hrgn)
	{
...
inline void CDrawContext::RemoveClip()
...
inline void CDrawContext::SelectFont( LPCTSTR pcszFontNames, int nSizePixels, int nWeight, bool bItalic, bool bUnderline, bool bStrike, BYTE cCharSet )
	{
		...
		FontDef fdef( pcszFontNames, nSizePixels, nWeight, bItalic, bUnderline, bStrike, cCharSet, false );
		SelectFont( fdef );
	}
...
inline int CDrawContext::ScaleX(int x) const
	{ return WinHelper::MulDiv(x, m_cxDeviceScaleNumer, m_cxDeviceScaleDenom); }


inline int CDrawContext::ScaleY(int y) const
	{ return WinHelper::MulDiv(y, m_cyDeviceScaleNumer, m_cyDeviceScaleDenom); }
