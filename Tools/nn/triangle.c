...

{
  ...
  fprintf(stderr, "triangle x%zux with orientation %d:\n", (size_t) t->tri,
         t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == m->dummytri) {
    fprintf(stderr, "    [0] = Outer space\n");
  } else {
    fprintf(stderr, "    [0] = x%zux  %d\n", (size_t) printtri.tri,
           printtri.orient);
  }
  ...
  if (printvertex == (vertex) NULL)
    ...
    fprintf(stderr, "    Origin[%d] = x%zux  (%.12g, %.12g)\n",
  ...
  if (printvertex == (vertex) NULL)
    ...
    fprintf(stderr, "    Dest  [%d] = x%zux  (%.12g, %.12g)\n",
  ...
  if (printvertex == (vertex) NULL)
    ...
    fprintf(stderr, "    Apex  [%d] = x%zux  (%.12g, %.12g)\n",
  ...
  if (b->usesegments) {
    ...
    if (printsh.ss != m->dummysub) {
      fprintf(stderr, "    [6] = x%zux  %d\n", (size_t) printsh.ss,
             printsh.ssorient);
    }
    sdecode(t->tri[7], printsh);
    if (printsh.ss != m->dummysub) {
      fprintf(stderr, "    [7] = x%zux  %d\n", (size_t) printsh.ss,
             printsh.ssorient);
    }
    sdecode(t->tri[8], printsh);
    if (printsh.ss != m->dummysub) {
      fprintf(stderr, "    [8] = x%zux  %d\n", (size_t) printsh.ss,
             printsh.ssorient);
    }
  }
...

{
  ...
  fprintf(stderr, "subsegment x%zux with orientation %d and mark %d:\n",
