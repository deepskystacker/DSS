...
#define TS 256		/* Tile Size */
	...
	void CLASS ahd_interpolate_mod()
	{
		...
		merror (buffer, "ahd_interpolate_mod()");
		rgb  = (void *) buffer;
				...
				for (row = top < 2 ? 2:top; row < top+TS && row < height-2; row++) 
				{
					...
					for (c = FC(row,col); col < left+TS && col < width-2; col+=2) 
					{
						...
						else 
						{
							...
							p2 = pix[ 0][c];
							p4 = pix[ 2][c];
							val = ((p1+p2+p3+1) >> 1) - ((p0+p4) >> 2);
							if (p1 > p3) 
							{
								if (val > p1 || val < p3)
									val = (ABS(p2-p0) < ABS(p2-p4)) ? 
									p1 + ((p2-p0+1) >> 1) : p3 + ((p2-p4+1) >> 1);
								if (val > p1)
									val = p1;
								else if (val < p3)
									val = p3;
				...
				for (d=0; d < 2; d++)
					for (row=top+1; row < top+TS-1 && row < height-1; row++)
						for (col=left+1; col < left+TS-1 && col < width-1; col++) 
						{
								...
								else 
								{
									...
									{  // Horizontal
										val = ((pix[-1][2-c] + pix[1][2-c] + 1) >> 1) +
											((2*pix[0][1] - rix[-1][1] - rix[1][1]) >> 2);
										if (pix[-1][2-c] > pix[1][2-c]) {
											if (val > pix[-1][2-c])
												val = pix[-1][2-c];
											else if (val < pix[1][2-c])
												val = pix[1][2-c]; 
										} 
										else {
											if (val < pix[-1][2-c])
								...
								else 
								{
									...
									else 
									{
										val = ((pix[-width][c] + pix[width][c] + 1) >> 1) + 
											((2*pix[0][1] - rix[-TS][1] - rix[TS][1]) >> 2);
										if (pix[-width][c] > pix[width][c]) {
											if (val > pix[-width][c])
												val = pix[-width][c];
											else if (val < pix[width][c])
												val = pix[width][c];
										}
										else {
											if (val < pix[-width][c])
							...
							else if (pix[-width-1][c] == pix[-width+1][c] &&
...
#undef TS
	...
	ANSI-C implementation of bilateral filtering in CIELAB space

	Written by Paul J. Lee (jaiyong dot lee at gmail o com)
	...
	Bombay, India. http://www.cse.ucsc.edu/~manduchi/Papers/ICCV98.pdf 
	*/
...
#define RGB2Lab_at_D65(_KK_,_JJ_,_II_) { \
	R = (double)image[_KK_][0]/(double)clip_max; \
	G = (double)image[_KK_][1]/(double)clip_max; \
	B = (double)image[_KK_][2]/(double)clip_max; \
	X = 0.43395275530903*R + 0.37621941468095*G + 0.18982783001002*B; \
	Y = 0.212671*R         + 0.715160*G         + 0.072169*B;         \
	Z = 0.01775791409262*R + 0.10947652086697*G + 0.87276556504040*B; \
	iV = V = X*lab_rLUT; fX = lab_LUT[iV][0] + lab_LUT[iV][1]*(V-iV); \
	iV = V = Y*lab_rLUT; fY = lab_LUT[iV][0] + lab_LUT[iV][1]*(V-iV); \
	iV = V = Z*lab_rLUT; fZ = lab_LUT[iV][0] + lab_LUT[iV][1]*(V-iV); \
	Lab_buf[_JJ_][_II_][0] = 116* fY - 16;  \
	Lab_buf[_JJ_][_II_][1] = 500*(fX - fY); \
	Lab_buf[_JJ_][_II_][2] = 200*(fY - fZ); }

	void CLASS bilateral_filter_mod()
	{
		double R, G, B, X, Y, Z, fX, fY, fZ, L, a, b;
		double scale_h, scale_s, scale_f, f, V, tmp1, tmp2;
		double **lab_LUT, **exp_LUT, ***Lab_buf, **L2h;
		int nh, nf, ii, jj, kk, ll, mm, nn, lab_nLUT, exp_nLUT, iV;
		double lab_rLUT, lab_dLUT, exp_rLUT, exp_dLUT, sigma_h, sigma_s;

		/* copy filter parameters */
		sigma_h     = sigma_d;
		sigma_s     = sigma_r;

		/* detemine size of filter window with 99% threshold */
		nh = sigma_h * sqrt(-log(0.01)) + 0.5;
		nf = 2 * nh + 1;
		if (verbose) fprintf(stderr,"Bilateral filtering in CIELAB space...\n");

		/* build Lab look-up-table */
		lab_nLUT = 1024;
		lab_rLUT = (double)lab_nLUT;
		lab_dLUT = 1.0/lab_rLUT;
		lab_LUT = (double **)alloc2(2,lab_nLUT+1,sizeof(double));
		lab_LUT[0][0] = 1.3793103448275862e-1;
		for (ll=1; ll < lab_nLUT+1; ll++) {
			V = lab_dLUT*(double)ll;
			if (V > 8.85645167903563082e-3) {
				if (V >= 0.5)
				{tmp1=3.3333333333333333333e-1; tmp2=V;}
				else
				{if (V >= 0.25)
				{tmp1=2.6456684199469991246e-1; tmp2=V*2.0;}
				else
				{if (V >= 0.125)
				{tmp1=2.0998684164914552746e-1; tmp2=V*4.0;}
				else
				{if (V >= 0.0625)
				{tmp1=1.6666666666666666667e-1; tmp2=V*8.0;}
				else 
				{if (V >= 0.03125)
				{tmp1=1.3228342099734995623e-1; tmp2=V*16.0;}
				else
				{if (V >= 0.015625)
				{tmp1=1.0499342082457276373e-1; tmp2=V*32.0;}
				else
				{tmp1=8.3333333333333333333e-2; tmp2=V*64.0;}}}}}}
				f = (2.9782632221671188188+14.250936501192798991*tmp2)
					/ (7.7291666666666666667+tmp2*(10.50-tmp2));
				f = 0.33333333333333333333*(2.0*f+tmp2/(f*f));
				lab_LUT[ll][0] = tmp1*(2.0*f+tmp2/(f*f));
			}
			else 
				lab_LUT[ll][0] = 7.787037037037037*V+1.3793103448275862e-1;
			lab_LUT[ll-1][1] = lab_LUT[ll][0]-lab_LUT[ll-1][0];
		}
		lab_LUT[lab_nLUT][1] = 0;

		/* build exponential look-up-table */
		exp_nLUT = 1024;
		exp_rLUT = 100.0;
		exp_dLUT = 1.0 / exp_rLUT;
		exp_LUT = (double **)alloc2(2,exp_nLUT,sizeof(double));
		exp_LUT[0][0] = 1;
		for (ll=1; ll < exp_nLUT; ll++) {
			exp_LUT[ll][0] = exp(-exp_dLUT*(double)ll);
			exp_LUT[ll-1][1] = exp_LUT[ll][0]-exp_LUT[ll-1][0];
		}
		exp_LUT[exp_nLUT-1][1] = exp(-exp_dLUT*exp_nLUT)-exp_LUT[exp_nLUT-1][0];

		/* allocate buffers */
		L2h     = (double **)alloc2(nf,nf,sizeof(double));
		Lab_buf = (double ***)alloc3(3,width,nf,sizeof(double));

		/* parameters */
		scale_h = 0.5/(sigma_h*sigma_h);
		scale_s = 0.5/(sigma_s*sigma_s);

		/* compute argument for Gaussian blur filter */
		for (mm = 0; mm < nf; mm++) {
			tmp1 = mm-nh;
			tmp1 *= tmp1;
			for (nn = 0; nn < nf; nn++) {
				tmp2 = nn-nh;
				L2h[mm][nn] = (tmp1 + tmp2*tmp2) * scale_h;
			}
		}

		/* the first nf pixel rows */
		for (kk=0, jj=nh; jj < nf; jj++)
			for (ii=0; ii < width; ii++, kk++)
				RGB2Lab_at_D65(kk,jj,ii);

		/* bilateral filtering loop */
		for (jj=0; jj < height; jj++) {
			for (ii=0; ii < width; ii++) {

				/* compute and apply bilateral filter in CIELAB space */
				scale_f = L = a = b = 0;
				for (nn=-nh; nn <= nh; nn++)
					if (jj+nn >= 0 && jj+nn < height)
						for (mm=-nh; mm <= nh; mm++)
							if (ii+mm >= 0 && ii+mm < width) {
								tmp1 = 0;
								for (kk=0; kk < 3; kk++) {
									tmp2 = Lab_buf[nh+nn][ii+mm][kk] - Lab_buf[nh][ii][kk];
									tmp1 += tmp2*tmp2;
								}
								iV = V = exp_rLUT*(L2h[nn+nh][mm+nh]+tmp1*scale_s);
								if (iV < exp_nLUT) {
									tmp1 = exp_LUT[iV][0] + exp_LUT[iV][1]*(V-iV);
									scale_f += tmp1;
									L += Lab_buf[nh+nn][ii+mm][0] * tmp1;
									a += Lab_buf[nh+nn][ii+mm][1] * tmp1;
									b += Lab_buf[nh+nn][ii+mm][2] * tmp1;
								}
							}
							L /= scale_f;
							a /= scale_f;
							...
							/* Lab => RGB @ D65 */
							/* XYZ */
							Y = ((fY=(L+16)/116) > 2.0689655172413793e-1 ? fY*fY*fY :
								1.107056459879453852e-3*L);
							X = ((fX=fY+0.002*a) > 2.0689655172413793e-1 ? fX*fX*fX :
								1.2841854934601665e-1*fX-1.7712903358071262e-2);
							Z = ((fZ=fY-0.005*b) > 2.0689655172413793e-1 ? fZ*fZ*fZ : 
								1.2841854934601665e-1*fZ-1.7712903358071262e-2);
							/* RGB */
							R = ((R= 3.07993493553299*X-1.53715151627130*Y-0.54278341926170*Z) > 1)
								? 1 : (R < 0 ? 0 : R);
							G = ((G=-0.92123418275394*X+1.87599000148989*Y+0.04524418126405*Z) > 1)
								? 1 : (G < 0 ? 0 : G);
							B = ((B= 0.05288968204587*X-0.20404133836651*Y+1.15115165632065*Z) > 1)
								? 1 : (B < 0 ? 0 : B);
							/* copy RGB */
							kk = jj*width+ii;
							image[kk][0] = R*(double)clip_max + 0.5;
							image[kk][1] = G*(double)clip_max + 0.5;
							image[kk][2] = B*(double)clip_max + 0.5;
							/* end of pixel column */
			}

			/* shifting Lab_buf */
			memcpy(Lab_buf[0][0],Lab_buf[1][0],3*width*(nf-1)*sizeof(double));

			/* compute next row's Lab value */
			if ((ll=jj+nh+1) < height) 
				for (ii=0; ii < width; ii++) {
					kk = ll*width + ii;
					RGB2Lab_at_D65(kk,nf-1,ii);
				}
		}

		/* free up buffer arrays */
		free2((void **)lab_LUT);
		free2((void **)exp_LUT);
		free2((void **)L2h);
		free3((void ***)Lab_buf);
	...
	int CLASS main (int argc, char **argv)
	{
		...
		if (argc == 1)
		{
			fprintf (stderr,
				...
				"\n-B <domain> <range>  Apply bilateral filter to reduce noise in CIELAB space"
...
next:
			...
			if (filters && !document_mode) {
				if (quality == 0)
					...
					vng_interpolate();
				else ahd_interpolate_mod();
			}
			if (sigma_d > 0 && sigma_r > 0) bilateral_filter_mod();
