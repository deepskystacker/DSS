...
TEST_CASE("BitMapFiller gray", "[BitMap][BitMapFiller][gray]")
{
	...
	BitMapFiller filler{ pBitmap, nullptr };
	filler.setGrey(true);
	...
	SECTION("Write 1 line 16 bps with 2 calls")
	{
		...
		REQUIRE_FALSE(memcmp(pGray->m_vPixels.data(), data, 16) == 0);
	}
	...
	SECTION("2 lines 16 bps and adjust CFA colors")
	{
		filler.setMaxColors(65535);
		filler.setWidth(8);
		filler.setHeight(2);
...
TEST_CASE("BitMapFiller color", "[BitMap][BitMapFiller][color]")
{
	...
	SECTION("Write 1 line 16 bps with 2 calls after RGB pixel")
	{
		...
		filler.Write(inputData, 2, 18);
		filler.Write(inputData + 18, 2, 6);
	...
	SECTION("Write 2 lines with 2 calls")
	{
		...
		filler.Write(inputData, 2, 24);
		filler.Write(inputData + 24, 2, 24);
	...
	SECTION("Write 1 line 16 bps with 1 call per pixel")
	{
		...
		for (int n = 0; n < 8; ++n)
			filler.Write(inputData + 3 * n, 2, 3);
