...
TEST_CASE("Register engine", "[Register][RegisterSubrect]")
{
	...
	SECTION("Block of 9 pixels")
	{
		...
		REQUIRE(stars.size() == 1);
		REQUIRE(stars.cbegin()->m_fX == Approx(116.0).epsilon(1e-8));
		REQUIRE(stars.cbegin()->m_fY == Approx(97.0).epsilon(1e-8));
	}
	...
	SECTION("Block of 9 and block of 4 pixels close together is one star")
	{
		...
		pGray->m_vPixels[95 * W + 117] = 180.0;
		...
		REQUIRE(stars.size() == 1);
		REQUIRE(stars.cbegin()->m_fX == Approx(115.73).epsilon(1e-2));
		REQUIRE(stars.cbegin()->m_fY == Approx(96.512).epsilon(1e-2));
	}
}
