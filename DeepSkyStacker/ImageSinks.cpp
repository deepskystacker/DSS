				...
				for (int i = 0; i < m_vRefStars.size(); i++)
				{
					if (IsRefStarVoted(i))
					{
						...
						if (rc.Width() > 10 && rc.Height() > 10)
						{
							...
							m_pImage->BitmapToScreen(fX, fY);
							pt.x = fX; pt.y = fY;
							graphics.DrawLine(&pen, static_cast<int>(pt.x)-5, static_cast<int>(pt.y), static_cast<int>(pt.x)+6, static_cast<int>(pt.y));
							graphics.DrawLine(&pen, static_cast<int>(pt.x), static_cast<int>(pt.y)-5, static_cast<int>(pt.x), static_cast<int>(pt.y)+6);
						};
					};
				};
			};
			...
			for (int i = 0; i < m_vStars.size(); i++)
			{
				if (IsTgtStarVoted(i) && !m_vStars[i].m_bRemoved)
				{
					...
					if (g_bShowRefStars && m_vRefStars.size())
					{
						...
						graphics.DrawLine(&pen, static_cast<int>(ptOrgBmp.x), static_cast<int>(ptOrgBmp.y), static_cast<int>(ptDstBmp.x), static_cast<int>(ptDstBmp.y));
					};
