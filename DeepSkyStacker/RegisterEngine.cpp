...
size_t CRegisteredFrame::RegisterSubRect(CMemoryBitmap* pBitmap, const DSSRect& rc, STARSET& stars)
{
	...
	const size_t width = rc.width();
	std::vector<double> values(width * rc.height());
	for (int j = rc.getTop(), ndx = 0; j < rc.getBottom(); ++j)
		for (int i = rc.getLeft(); i < rc.getRight(); ++i, ++ndx)		
			pBitmap->GetPixel(i, j, values[ndx]);

	const size_t top = rc.getTop();
	const size_t left = rc.getLeft();
	const auto getValue = [&values, width, top, left](const size_t x, const size_t y) -> double
	{
		return values[(y - top) * width + x - left];
	};
					...
					if (fIntensity >= m_fMinLuminancy + m_fBackground)
					{
						...
						if (bNew)
						{
							...
							if (!bMainOk && !bBrighterPixel && (lMaxRadius > 2))
							{
								...
								rcStar.setCoords(ptTest.x() - lLeftRadius, ptTest.y() - lTopRadius, ptTest.x() + lRightRadius, ptTest.y() + lBottomRadius);
...
bool	CRegisteredFrame::SaveRegisteringInfo(LPCTSTR szInfoFileName)
{
	...
	if (hFile)
	{
		...
		for (int i = 0; i<m_vStars.size();i++)
		{
			...
			fprintf(hFile, "Rect = %ld, %ld, %ld, %ld\n", m_vStars[i].m_rcStar.getLeft(), m_vStars[i].m_rcStar.getTop(), m_vStars[i].m_rcStar.getRight(), m_vStars[i].m_rcStar.getBottom());
...
bool CLightFrameInfo::ComputeStarShifts(CMemoryBitmap* pBitmap, CStar& star, double& fRedXShift, double& fRedYShift, double& fBlueXShift, double& fBlueYShift)
{
	...
	for (int j = star.m_rcStar.getTop(); j <= star.m_rcStar.getBottom(); j++)
	{
		fSumRedX = 0;
		fNrValuesRedX = 0;
		fSumBlueX = 0;
		fNrValuesBlueX = 0;
		for (int i = star.m_rcStar.getLeft(); i <= star.m_rcStar.getRight(); i++)
		{
	...
	for (int j = star.m_rcStar.getLeft(); j <= star.m_rcStar.getRight(); j++)
	{
		fSumRedY = 0;
		fNrValuesRedY = 0;
		fSumBlueY = 0;
		fNrValuesBlueY = 0;
		for (int i = star.m_rcStar.getTop(); i <= star.m_rcStar.getBottom(); i++)
		{
			double fRed, fGreen, fBlue;
